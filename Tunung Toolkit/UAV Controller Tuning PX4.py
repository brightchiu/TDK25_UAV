...
class Parameter:
    def __init__(self):
        self.value = {
            ...
            'CONN_UAV_TIMEOUT': 30,
            'CONN_UAV_PORT': '/dev/cu.usbmodem14601',
        }
...
class TCPTransmit:
    ...
    async def enable_sever(self):
        print('Creat sever')
        self.server = await asyncio.start_server(self.sever_loop, '192.168.137.246', 5000)
...
class UAVTuning:
    ...
    def msg_to_attitude_target(self, x, name, msg):
        ...
        if self.STATE.TEST_MODE == 0:
            self.CMD.ROLL_RATE = math.degrees(float(msg.body_roll_rate))
        if self.STATE.TEST_MODE == 1:
            self.CMD.PITCH_RATE = math.degrees(float(msg.body_pitch_rate))
        if self.STATE.TEST_MODE == 2:
            self.CMD.YAW_RATE = math.degrees(float(msg.body_yaw_rate))
